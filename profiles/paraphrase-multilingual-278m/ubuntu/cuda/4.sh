...
# Model: paraphrase-multilingual-278m  Lane: ubuntu/cuda
set -euo pipefail
PROFILE_STEP="4"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LANE_JSON="${SCRIPT_DIR}/lane.json"
PROFILE_SIZES="${SCRIPT_DIR}/profile-sizes.csv"
MODEL_SLUG="paraphrase-multilingual-278m"
TARGET_LANE="ubuntu/cuda"
if [[ ! -f "${LANE_JSON}" ]]; then
  echo "Missing lane metadata: ${LANE_JSON}" >&2
  exit 1
fi
echo "[profile-step-${PROFILE_STEP}] ${MODEL_SLUG} / ${TARGET_LANE} — Hard Disk Gate"
